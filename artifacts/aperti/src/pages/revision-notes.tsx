    ...
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden">
      ...
      <div className="w-72 border-r border-border bg-card flex flex-col">
        <div className="p-4 border-b border-gray-100 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="font-bold text-gray-900 flex items-center gap-2">
              <BookOpen className="w-4 h-4 text-primary" /> Revision Notes
            </h2>
            <button onClick={() => createMut.mutate()} disabled={createMut.isPending}
              className="w-7 h-7 bg-primary text-white rounded-lg flex items-center justify-center hover:bg-primary transition-colors">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/30"
            />
          </div>
          {/* AI generate */}
          <button onClick={() => setShowAiForm(v => !v)}
            className="w-full flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-primary to-emerald-50 border border-primary/15 text-primary rounded-xl text-sm font-medium hover:border-primary/25 transition-colors">
              ...
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                <div className="space-y-2">
                  ...
                  <button onClick={() => aiMut.mutate()} disabled={!aiTopic.trim() || aiMut.isPending}
                    className="w-full py-2 bg-primary text-white rounded-xl text-sm font-semibold hover:bg-primary disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
        ...
        <div className="flex-1 overflow-y-auto">
          {isLoading ? (
            <div className="flex justify-center py-10"><div className="w-5 h-5 border-2 border-primary/60 border-t-transparent rounded-full animate-spin" /></div>
          ) : filtered.length === 0 ? (
            <div className="py-2">
              <AppEmptyState
            ...
            <div className="divide-y divide-gray-50">
              {filtered.map((note: any) => (
                <button key={note.id} onClick={() => { setSelected(note); setEditing(false); }}
                  className={`w-full text-left px-4 py-3 hover:bg-gray-50 transition-colors ${selected?.id === note.id ? "bg-primary/8" : ""}`}>
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <p className={`text-sm font-medium truncate ${selected?.id === note.id ? "text-primary" : "text-gray-800"}`}>
                        {note.title || "Untitled"}
                      </p>
                    ...
                    {note.ai_generated && <Sparkles className="w-3 h-3 text-primary shrink-0 mt-0.5" />}
                  </div>
                </button>
              ))}
            </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
        {selected ? (
            ...
            <div className="bg-card border-b border-border px-6 py-3 flex items-center justify-between gap-4">
              {editing ? (
                <input
                  ...
                  className="flex-1 text-lg font-bold text-gray-900 bg-transparent focus:outline-none border-b-2 border-primary/40"
                />
              ) : (
                ...
                {editing ? (
                  <>
                    <button onClick={() => updateMut.mutate()} disabled={updateMut.isPending}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-primary text-white rounded-lg text-sm font-semibold hover:bg-primary disabled:opacity-50 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {editing ? (
                <textarea
                  ...
                  className="w-full h-full min-h-[400px] bg-card border border-border rounded-xl p-5 text-sm font-mono text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none shadow-sm"
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-10">
            ...
            <button onClick={() => createMut.mutate()} disabled={createMut.isPending}
              className="flex items-center gap-2 px-5 py-2.5 bg-primary text-white rounded-xl text-sm font-semibold hover:bg-primary transition-colors">
