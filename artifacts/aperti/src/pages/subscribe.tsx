...
export default function SubscribePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-primary/40 via-white to-gray-50/30 py-10 px-4">
              ...
              <div className="flex flex-col items-center gap-1.5">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${
                  i < stepIndex
                    ...
                    : i === stepIndex
                      ? "bg-primary text-white ring-4 ring-primary/30 shadow-md shadow-primary/20"
                      : "bg-white text-gray-400 border border-gray-200"
                }`}>
                  {i < stepIndex ? <Check className="w-3.5 h-3.5" /> : i + 1}
                </div>
                <span className={`text-[10px] font-semibold tracking-wide ${i === stepIndex ? "text-primary" : i < stepIndex ? "text-primary" : "text-gray-400"}`}>
                  {s.label}
                </span>
              </div>
              {i < steps.length - 1 && (
                <div className={`w-12 h-0.5 mb-5 mx-1 transition-colors duration-300 ${i < stepIndex ? "bg-primary/70" : "bg-gray-200"}`} />
              )}
            </div>
          ))}
        </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-7">
                ...
                {plansLoading ? (
                  ...
                  <div className="text-center py-10">
                    ...
                    <button onClick={() => navigate("/pricing")} className="mt-4 text-primary hover:underline text-sm inline-flex items-center gap-1">
                      <ArrowLeft className="w-3.5 h-3.5" /> Browse plans
                    </button>
                  </div>
                ) : (
                  <>
                    <div className="p-5 bg-gradient-to-br from-primary to-emerald-50/60 rounded-xl border border-primary/15 flex items-center justify-between mb-5">
                      <div>
                        <p className="font-bold text-foreground text-lg">{plan.name}</p>
                        <p className="text-sm text-primary capitalize mt-0.5">{plan.type} plan</p>
                      </div>
                      <div className="text-right">
                        <p className="text-2xl font-bold text-primary">{Number(plan.price_egp).toLocaleString()}<span className="text-sm font-normal ml-1">EGP</span></p>
                        <p className="text-xs text-primary mt-0.5">per month</p>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full py-3.5 bg-primary hover:bg-primary active:bg-primary/80 text-white rounded-xl font-semibold flex items-center justify-center gap-2 transition-all duration-150 disabled:opacity-60 shadow-sm shadow-primary/20 hover:shadow-md hover:shadow-primary/20 text-sm"
                    >
            ...
            <motion.div key="payment" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-7">
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="flex items-center justify-between p-4 bg-primary/8 border border-primary/25 rounded-xl">
                    <div>
                      <p className="text-[10px] text-primary uppercase font-bold tracking-wider mb-1">Reference Code</p>
                      <p className="font-mono font-bold text-foreground text-xl">{paymentRequest.instapay?.reference}</p>
                    </div>
                    <button
                      onClick={() => copyRef(paymentRequest.instapay?.reference)}
                      className="flex items-center gap-1.5 text-xs text-primary hover:text-foreground border border-primary/25 bg-white px-3 py-2 rounded-lg transition-colors hover:bg-primary/8"
                    >
                ...
                <button
                  ...
                  className="w-full mt-5 py-3.5 bg-primary hover:bg-primary text-white rounded-xl font-semibold transition-all text-sm shadow-sm shadow-primary/20"
                >
            ...
            <motion.div key="proof" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-7">
                ...
                <input
                  ...
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-transparent transition-all"
                />
                ...
                <button
                  ...
                  className="w-full mt-5 py-3.5 bg-primary hover:bg-primary text-white rounded-xl font-semibold flex items-center justify-center gap-2 transition-all disabled:opacity-50 text-sm shadow-sm shadow-primary/20"
                >
                  {proofMut.isPending ? (
            ...
            <motion.div key="confirmed" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-10 text-center">
                <motion.div
                  ...
                  className="w-18 h-18 bg-gradient-to-br from-primary to-emerald-100 rounded-full flex items-center justify-center mx-auto mb-5"
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                  <button onClick={() => navigate("/")}
                    className="px-6 py-2.5 bg-primary text-white rounded-xl text-sm font-semibold hover:bg-primary transition-colors shadow-sm">
                    Go to Dashboard
                  </button>
