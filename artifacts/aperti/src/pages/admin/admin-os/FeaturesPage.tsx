...
function FlagRow({ flag, onToggle, onEdit }: any) {
  return (
    <motion.div layout className="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-4">
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${flag.enabled ? "bg-primary/8" : "bg-gray-50"}`}>
        <Zap className={`w-5 h-5 ${flag.enabled ? "text-primary" : "text-gray-400"}`} />
      </div>
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        <button
          ...
          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${flag.enabled ? "bg-primary" : "bg-gray-200"}`}
        >
        ...
        <button onClick={() => onEdit(flag)} className="text-xs text-gray-500 hover:text-primary px-2 py-1 border border-gray-200 rounded-lg hover:border-primary/25 transition-colors">Edit</button>
      </div>
    </motion.div>
  );
}
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setEditing(null); setForm({ name: "", description: "", enabled: false, status: "enabled", targetRoles: "" }); setShowCreate(true); }} className="flex items-center gap-2 px-4 py-2 text-sm bg-primary text-white rounded-lg hover:bg-primary/80 transition-colors">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4">
        ...
        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full bg-primary rounded-full transition-all" style={{ width: flagList.length ? `${(enabledCount / flagList.length) * 100}%` : "0%" }} />
        </div>
      </div>
          ...
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={form.name} onChange={(e) => setForm(p => ({ ...p, name: e.target.value }))} disabled={!!editing} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-primary/60 disabled:bg-gray-50" placeholder="e.g. live_classes" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Description</label>
                <input value={form.description} onChange={(e) => setForm(p => ({ ...p, description: e.target.value }))} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-primary/60" placeholder="Brief description…" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Status</label>
                <select value={form.status} onChange={(e) => setForm(p => ({ ...p, status: e.target.value }))} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-primary/60">
              ...
              <div>
                ...
                <input value={form.targetRoles} onChange={(e) => setForm(p => ({ ...p, targetRoles: e.target.value }))} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-primary/60" placeholder="teacher, student, admin" />
              </div>
              <div className="flex items-center gap-3">
                <label className="text-sm font-medium text-gray-700">Enabled</label>
                <button onClick={() => setForm(p => ({ ...p, enabled: !p.enabled }))} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${form.enabled ? "bg-primary" : "bg-gray-200"}`}>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleSave} disabled={saveMutation.isPending} className="flex-1 px-4 py-2 text-sm bg-primary text-white rounded-lg hover:bg-primary/80 disabled:opacity-50">
