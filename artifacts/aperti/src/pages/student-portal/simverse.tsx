...
Be encouraging and educational. Keep response under 300 words.`;
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3 p-4 bg-gradient-to-r from-primary to-emerald-50 rounded-2xl border border-primary/15">
          ...
          <div key={s} className="flex items-center gap-2">
            <button onClick={() => setStep(s as 1 | 2 | 3)}
              className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${
                step === s ? "text-white" : step > s ? "bg-primary/15 text-primary" : "bg-gray-100 text-gray-400"
              }`}
              style={step === s ? { background: "hsl(var(--primary))" } : {}}>
              {s}
            </button>
            <span className={`text-xs ${step === s ? "text-primary font-semibold" : "text-gray-400"}`}>
        ...
        {step === 1 && (
          <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            <div className="bg-card rounded-2xl border border-border shadow-sm p-5 space-y-3">
              ...
              />
              <Button onClick={() => setStep(2)} disabled={!hypothesis.trim()} className="w-full rounded-xl" style={{ background: "hsl(var(--primary))" }}>
                Next: Procedure →
              </Button>
            </div>
          </motion.div>
          ...
          <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            <div className="bg-card rounded-2xl border border-border shadow-sm p-5 space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={() => setStep(3)} disabled={!procedure.trim()} className="flex-1 rounded-xl" style={{ background: "hsl(var(--primary))" }}>
                  Next: Observations →
                </Button>
              </div>
            </div>
      ...
      <AnimatePresence>
        {result && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            className="bg-gradient-to-br from-primary to-emerald-50 rounded-2xl border border-primary/15 p-5">
            <div className="flex items-center gap-2 mb-3">
              <div className="h-7 w-7 rounded-lg bg-primary flex items-center justify-center">
                <Sparkles className="h-3.5 w-3.5 text-white" />
              </div>
              <h4 className="font-bold text-foreground text-sm">Mentor Evaluation</h4>
            </div>
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <div className="flex items-center gap-3">
          <div className="h-9 w-9 rounded-xl bg-primary/8 flex items-center justify-center">
            <FlaskConical className="h-4.5 w-4.5 text-primary" style={{ width: 18, height: 18 }} />
          </div>
          <div>
