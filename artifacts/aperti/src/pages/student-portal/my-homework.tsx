    ...
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
          <BookOpen className="h-6 w-6 text-indigo-500" />Homework
        </h1>
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
        {([["pending", "Pending", pending.length], ["submitted", "Submitted", submitted.length], ["graded", "Graded", graded.length]] as [typeof activeTab, string, number][]).map(([tab, label, count]) => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            ...
            {label}
            {count > 0 && <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${activeTab === tab ? "bg-indigo-100 text-indigo-700" : "bg-gray-200 text-gray-600"}`}>{count}</span>}
          </button>
        ))}
      </div>
            ...
            return (
              <motion.div key={hw.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                ...
                <button className="w-full flex items-start gap-4 p-5 text-left hover:bg-gray-50/50 transition-colors"
                  ...
                  <div className={`w-1 self-stretch rounded-full flex-shrink-0 ${isOverdue ? "bg-red-400" : status?.color.replace("text-", "bg-").replace("-600", "-400") || "bg-indigo-300"}`} />
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-gray-400">
                      {hw.subjectName && <span className="text-indigo-500 font-medium">{hw.subjectName}</span>}
                      {hw.dueDate && (
                      ...
                      <div className="px-5 pb-5 space-y-4 border-t border-gray-50 pt-4">
                        ...
                        {hw.instructions && (
                          <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-3">
                            <p className="text-xs font-semibold text-indigo-700 mb-1">Instructions</p>
                            <p className="text-xs text-indigo-600 whitespace-pre-wrap">{hw.instructions}</p>
                          </div>
                        )}
                        ...
                        {hw.submissionStatus === "graded" ? (
                          <div className="space-y-3">
                            ...
                            {hw.submissionContent && <div className="bg-gray-50 rounded-xl p-3"><p className="text-xs text-gray-500 font-medium mb-1">Your submission</p><p className="text-xs text-gray-700 whitespace-pre-wrap">{hw.submissionContent}</p></div>}
                          </div>
                        ) : (
                          <div className="space-y-3">
                            <Textarea
                              className="text-sm min-h-[100px] resize-none border-gray-200 rounded-xl focus:border-indigo-300"
                            ...
                            <div className="flex gap-2">
                              ...
                              <Button size="sm" className="gap-2 text-xs bg-indigo-600 hover:bg-indigo-700" disabled={submitting === hw.id || !drafts[hw.id]?.trim()} onClick={() => handleSubmit(hw.id, false)}>
