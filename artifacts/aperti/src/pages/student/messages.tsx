...
import { MessageSquare, Send, Bell, Calendar, ChevronRight } from "lucide-react";
...
function ThreadView({ threadId, onBack }: { threadId: number; onBack: () => void }) {
  ...
  const sendMutation = useMutation({
    mutationFn: (content: string) => postJSON(`/api/messages/threads/${threadId}/send`, { content }),
    onSuccess: () => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)]">
      <div className="flex items-center gap-3 mb-4">
        <Button variant="ghost" size="sm" onClick={onBack} className="gap-1 text-muted-foreground">← Back</Button>
      ...
      <Card className="flex-1 flex flex-col shadow-sm overflow-hidden">
        <CardContent className="flex-1 flex flex-col p-0 overflow-hidden">
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {isLoading ? <Skeleton className="h-20 rounded-xl" /> : (messages ?? []).map((msg: any) => {
              const isMe = msg.senderId === user?.id;
              return (
                <div key={msg.id} className={`flex gap-2 ${isMe ? "justify-end" : ""}`}>
                  {!isMe && <Avatar className="h-7 w-7 shrink-0"><AvatarFallback className="text-[10px] bg-muted">U</AvatarFallback></Avatar>}
                  <div className={`rounded-2xl px-3 py-2 max-w-[75%] text-sm ${isMe ? "bg-primary text-white" : "bg-muted"}`}>
                    <p className="leading-relaxed">{msg.content}</p>
                    <p className={`text-[10px] mt-0.5 ${isMe ? "text-white/70" : "text-muted-foreground"}`}>
                      {msg.sentAt ? new Date(msg.sentAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : ""}
                    </p>
                  </div>
                  {isMe && <Avatar className="h-7 w-7 shrink-0"><AvatarFallback className="text-[10px] bg-primary text-white">ME</AvatarFallback></Avatar>}
                </div>
              );
            })}
            <div ref={scrollRef} />
          ...
          <div className="p-3 border-t flex gap-2">
            <Input
              ...
              onKeyDown={e => e.key === "Enter" && !e.shiftKey && text.trim() && sendMutation.mutate(text.trim())}
              className="flex-1 text-sm"
            />
            <Button size="sm" onClick={() => text.trim() && sendMutation.mutate(text.trim())} disabled={!text.trim() || sendMutation.isPending}>
      ...
      <Tabs defaultValue="messages">
        <TabsList className="mb-5">
          <TabsTrigger value="messages" className="gap-1.5">
            ...
            {(threads ?? []).length > 0 && <Badge className="h-4 min-w-4 text-[9px] p-0 flex items-center justify-center">{(threads ?? []).length}</Badge>}
          </TabsTrigger>
          ...
          <div className="space-y-3">
            {threadsLoading ? (
              ...
              (threads ?? []).map((thread: any) => (
                <motion.div key={thread.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                  <Card className="shadow-sm hover:shadow-md transition-shadow cursor-pointer" onClick={() => setSelectedThread(thread.id)}>
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between gap-3 mb-2">
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          {a.subject_name && <Badge variant="secondary" className="text-[10px]">{a.subject_name}</Badge>}
                          <Badge variant="outline" className="text-[10px] capitalize">{a.audience}</Badge>
                        </div>
                      </div>
                      <p className="text-sm text-muted-foreground leading-relaxed">{a.body}</p>
                      <p className="text-[11px] text-muted-foreground mt-2">
                        {a.sent_at ? new Date(a.sent_at).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" }) : "Not yet sent"}
                      </p>
                    </CardContent>
                  </Card>
                </motion.div>
                  ...
                  <Card className="shadow-sm">
                    <CardContent className="p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{event.time ?? ""} {event.type ? `· ${event.type}` : ""}</p>
                      </div>
                      {event.type && (
