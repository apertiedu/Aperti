...
const PHASE_FIXES_MIGRATIONS: string[] = [
  ...
  /* ── Features 26–34: Billing Intelligence Layer ─────────────────────── */
  `CREATE TABLE IF NOT EXISTS billing_events (
    id          bigserial PRIMARY KEY,
    type        text NOT NULL,
    entity_id   text NOT NULL,
    entity_type text NOT NULL DEFAULT 'subscription',
    user_id     integer REFERENCES accounts(id) ON DELETE SET NULL,
    payload     jsonb NOT NULL DEFAULT '{}',
    created_at  timestamptz NOT NULL DEFAULT NOW()
  )`,
  ...
  `CREATE INDEX IF NOT EXISTS idx_billing_events_entity ON billing_events(entity_id, entity_type)`,
  ...
  `CREATE INDEX IF NOT EXISTS idx_plan_changes_status ON plan_changes(status, created_at DESC)`,
  ...
  `CREATE INDEX IF NOT EXISTS idx_payment_recoveries_status ON payment_recoveries(status, next_retry_at)`,
  ...
  `CREATE INDEX IF NOT EXISTS idx_exp_assign_exp  ON experiment_assignments(experiment_id)`,
  ...
  `CREATE TABLE IF NOT EXISTS subscription_metrics_snapshots (
    ...
    trial_converted integer NOT NULL DEFAULT 0,
    conversion_rate numeric(6,4) NOT NULL DEFAULT 0,
    created_at      timestamptz NOT NULL DEFAULT NOW(),
    UNIQUE(snapshot_date)
  )`,
  `ALTER TABLE subscription_plans ADD COLUMN IF NOT EXISTS scope      text NOT NULL DEFAULT 'platform'`,
  `ALTER TABLE subscription_plans ADD COLUMN IF NOT EXISTS teacher_id integer REFERENCES accounts(id) ON DELETE CASCADE`,
  `ALTER TABLE subscription_plans ADD COLUMN IF NOT EXISTS course_id  integer`,
