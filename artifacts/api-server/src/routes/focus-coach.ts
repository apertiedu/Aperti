  ...
  try {
    ...
    for (const topic of weakTopics.slice(0, 2)) {
      ...
      if (!autoGoalTitles.has(title)) {
        autoGoals.push({ id: null, title, type: "daily", source: "auto", targetDate: today, xpReward: 50, completedAt: null, generated: true });
      }
    }
    ...
    for (const hw of pendingHw.slice(0, 2)) {
      ...
      if (!autoGoalTitles.has(title)) {
        autoGoals.push({ id: null, title, type: "daily", source: "auto", targetDate: today, xpReward: 75, completedAt: null, generated: true });
      }
    }

    const focusTitle = "Complete a 25-minute focus session";
    if (!autoGoalTitles.has(focusTitle) && !daily.find(g => g.title === focusTitle)) {
      autoGoals.push({ id: null, title: focusTitle, type: "daily", source: "auto", targetDate: today, xpReward: 30, completedAt: null, generated: true });
    }

    const allDaily = [...daily, ...autoGoals.filter(ag => !autoGoalTitles.has(ag.title))];
...
router.patch("/focus-sessions/:id", ...studentGuard, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const [updated] = await db.update(focusSessionsTable)
      .set({ durationMinutes: finalDuration, mode: mode ?? existing.mode, xpEarned, completedAt: new Date() })
